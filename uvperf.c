...
BOOL WaitForTestTransfer(PUVPERF_TRANSFER_PARAM transferParam, UINT msToWait) {
    ...
    while (transferParam) {
        ...
        LOG_MSG("waiting for Ep%02Xh thread..\n", transferParam->Ep.PipeId); //TODO Error occurs when 2 2 on writing
...
//showing descriptors
    ...
    ShowMenu();
    LOG_VERBOSE("Enter your choice: ");
    key = _getch();
    if (key != '\r') {  // If not Enter key
        switch (key) {
            case 'e':
                ShowEndpointDescriptor(libusb_get_device(handle), 0, 0);
                break;
            case 'i':
            //interface0 set as bulk transfer
            //interface1 set as isochronous transfer
                ShowInterfaceDescriptor(libusb_get_device(handle), 0);
                break;
            case 'c':
